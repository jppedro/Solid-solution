...
class Sis :
    def __init__ ( self ) :
        ...
        # cria tabela
        self . c . execute ('''CREATE TABLE IF NOT EXISTS ped (id INTEGER PRIMARY KEY , cli
TEXT , itens TEXT , tot REAL , st TEXT , dt TEXT , tp TEXT )''')
        self . db . commit ()
    def add_ped ( self , n , its , t ) :
        ...
        self . c . execute ("INSERT INTO ped (cli , itens , tot , st , dt , tp) VALUES (? , ? , ? ,? , ? , ?)", (n , its_str , tot , 'pendente ', dt , t ))
        self . db . commit ()
        # envia email
    ...
    def get_ped ( self , id) :
        ...
        r = self . c . fetchone ()
        if r :
            return {'id ': r [0] , 'cli ': r [1] , 'itens ': json . loads ( r [2]) , 'tot ': r [3] , 'st ': 
r [4] , 'dt ': r [5] , 'tp ': r [6]}
        return None
    ...
    def gerar_rel ( self , tipo ) :
        ...
        if tipo == 'vendas ':
            ...
            rs = self . c . fetchall ()
            print ("=== RELAT ´ORIO DE VENDAS ===")
            tot_g = 0
            for r in rs :
                print ( f" Pedido #{r [0]} - Cliente : {r [1]} - Total : R${r [3]:.2 f} - Status :{r [4]} ")
                tot_g += r [3]
    ...
    def proc_pag ( self , id , m , vl ) :
        ...
        if m == 'cartao ':
            print (" Processando pagamento com cart ~ao...")
            # valida cartao ( simplificado )
            print (" Cart ~ao validado !")
            ...
            print ("M´e todo de pagamento inv´a lido !")
            return False
    def validar_estoque ( self , its ) :
        ...
        for i in its :
            if i ['nome '] not in est :
                print ( f" Produto {i[ ' nome ']} n~ao encontrado !")
                return False
            if est [ i ['nome ']] < i ['q']:
...
def main () :
    ...
    if s . validar_estoque ( its1 ) :
        id1 = s . add_ped ('Jo~ao Silva ', its1 , 'normal ')
